...
func NewCountdownService(taskService *task.TaskService) *CountdownService {
...
func (cs *CountdownService) StartCountdown(task *task.Task, countdownMinutes int) {
	...
	cs.taskService.UpdateTaskSpentTime(task.Id, countdownMinutes*60-remainingSeconds) // Save the full duration
	close(cs.DoneChan)                                                                // Signal that the countdown has ended
}
